...
Available commands (run from the repo root):
  ...
  Final production model (validation-based checkpoint selection + calibration)
  ------------------------------------------------------------------------------
  jupyter notebook notebooks/train_final_model.ipynb
  # or, single (model, seed) run via CLI:
  python -m src.training.train_transformer --data_dir data/fakeddit --model_name distilbert-base-uncased --out_dir results/final_model --seed 42 --save_model

  Demo
  ----
  python app/demo_app.py --model_dir results/bert_base/checkpoint        # original Gradio demo
  streamlit run app/streamlit_app.py                                     # new Streamlit demo (loads from Hugging Face Hub)

See NEXT_STEPS.md for the full walkthrough, including where to get the data, uploading to Hugging Face, and deploying to Streamlit Community Cloud.
"""
